...
export default class Home extends React.Component {
    ...
    if (this.formIsValid()) {
      ...
      this.newSession()
        .then(data => {
          ...
          .then(response => {
            this.setState({sessionCreated: true, session_id: data.session_id})
          })
  ...
  render() {
    ...
    return (
      <div className="bg-gray-50 min-h-screen px-5 text-green-800">
        {this.state.sessionCreated ? (<Redirect to={'/' + this.state.session_id} />) : ""}
        ...
        <h1 className="text-3xl mb-4 font-black max-w-screen-sm mx-auto">
          Start a new brainstorming session
        </h1>
        <form className="max-w-screen-sm mx-auto" onSubmit={this.handleSubmit}>
          <div>
          ...
          <div className="flex items-center my-5 text-green-800">
            ...
            <p>Please note, you cannot change these fields after your session has started.</p>
          </div>
          <div className="flex justify-center mt-8">
            <button className="w-full sm:w-auto bg-green-500 px-10 py-5 text-xl tracking-wide font-bold text-white hover:bg-green-700 rounded">Start Session</button>
          </div>
        </form>
      </div>
    )
